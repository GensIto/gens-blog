import { createRoute } from "honox/factory";
import { Timeline, TIMELINE_DATA } from "../components/Timeline";

export default createRoute((c) => {
  return c.render(
    <div class='min-h-screen bg-gradient-to-br from-base-200 via-base-300 to-base-200'>
      {/* Hero Section */}
      <div class='container mx-auto px-4 py-12'>
        <div class='max-w-6xl mx-auto'>
          {/* Bento Grid Layout */}
          <div class='grid grid-cols-1 md:grid-cols-12 gap-4 mb-8'>
            {/* Profile Card - Large */}
            <div class='md:col-span-7 card bg-base-100 shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1'>
              <div class='card-body'>
                <div class='flex flex-col md:flex-row gap-6 items-center md:items-start'>
                  <div class='avatar'>
                    <div class='w-32 rounded-full'>
                      <img
                        src='/gens.JPG'
                        alt='Profile'
                      />
                    </div>
                  </div>
                  <div class='flex-1 text-center md:text-left'>
                    <h1 class='text-4xl font-bold mb-2'>Hi, I'm Gens</h1>
                    <div class='flex flex-wrap gap-2 justify-center md:justify-start'>
                      <div class='badge badge-primary badge-sm'>TypeScript</div>
                      <div class='badge badge-primary badge-sm'>Hono</div>
                      <div class='badge badge-primary badge-sm'>React</div>
                      <div class='badge badge-primary badge-sm'>Next.js</div>
                      <div class='badge badge-primary badge-sm'>Cloudflare</div>
                      <a
                        href='https://github.com/GensIto'
                        target='_blank'
                        rel='noopener noreferrer'
                        class='transition-all duration-300 hover:-translate-y-1 hover:scale-105'
                        style='transform-style: preserve-3d;'
                        title='GitHub'
                      >
                        <svg
                          xmlns='http://www.w3.org/2000/svg'
                          width='24'
                          height='24'
                          viewBox='0 0 24 24'
                          fill='currentColor'
                        >
                          <path d='M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z' />
                        </svg>
                      </a>
                    </div>
                  </div>
                </div>
              </div>
            </div>

            {/* Quick Links - Vertical */}
            <div class='md:col-span-5 grid grid-cols-2 md:grid-cols-1 gap-4'>
              <a
                href='/posts'
                class='card from-primary bg-base-100 to-primary-focus text-primary-content shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 hover:scale-105'
                style='transform-style: preserve-3d;'
              >
                <div class='card-body p-6'>
                  <h3 class='card-title text-lg'>ブログ</h3>
                  <p class='text-sm opacity-90'>技術記事を読む</p>
                </div>
              </a>
            </div>
          </div>

          {/* Timeline Section */}
          <div class='card bg-base-100 shadow-xl mb-8'>
            <div class='card-body'>
              <h2 class='card-title text-2xl mb-6'>職歴</h2>
              <Timeline data={TIMELINE_DATA} />
            </div>
          </div>
        </div>
      </div>
    </div>
  );
});
